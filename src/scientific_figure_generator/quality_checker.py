...
Implements quality gates inspired by Imbad0202 academic-research-skills and K-Dense-AI scientific-agent-skills.
Checks: resolution, colorblind safety, font embedding, axis completeness, label readability, panel label consistency.
"""
...
class FigureQualityChecker:
    ...
    def __init__(self, journal="nature", min_dpi=300, min_font_size=5):
        ...
        self.colorblind_palettes = ["viridis", "magma", "inferno", "plasma", "cividis", "coolwarm", "RdBu", "PiYG"]
    ...
    def _check_resolution(self, fig):
        ...
        dpi = fig.dpi
        if dpi < self.min_dpi:
            issues.append(QualityIssue("warning", "resolution", f"Figure DPI is {dpi}, minimum recommended is {self.min_dpi}", f"Set dpi={self.min_dpi} or higher in savefig"))
        w, h = fig.get_size_inches()
        if w < 2 or h < 2:
            issues.append(QualityIssue("error", "resolution", f"Figure size too small: {w:.1f}x{h:.1f} inches", "Increase figure size to at least 3x3 inches"))
        if w > 10 or h > 10:
            issues.append(QualityIssue("info", "resolution", f"Figure size large: {w:.1f}x{h:.1f} inches", "Consider if this fits journal column width"))
        return issues

    def _check_fonts(self, fig):
        issues = []
        if matplotlib.rcParams.get("pdf.fonttype") != 42:
            issues.append(QualityIssue("warning", "fonts", "PDF fonttype is not 42 (TrueType), text may not be editable", "Set plt.rcParams['pdf.fonttype'] = 42"))
        for text in fig.findobj(plt.Text):
            fs = text.get_fontsize()
            if isinstance(fs, (int, float)) and fs < self.min_font_size:
                issues.append(QualityIssue("warning", "fonts", f"Text '{text.get_text()[:30]}' has fontsize {fs}", f"Increase fontsize to at least {self.min_font_size}"))
                break
        return issues
            ...
            if ax.axison:
                if not ax.get_xlabel() and ax.get_xscale() != "log":
                    if len(ax.images) == 0 and len(ax.patches) < 3:
                        issues.append(QualityIssue("info", "axes", "Axes missing xlabel", "Add xlabel or verify this is intentional"))
                if not ax.get_ylabel() and ax.get_yscale() != "log":
                    if len(ax.images) == 0 and len(ax.patches) < 3:
                        issues.append(QualityIssue("info", "axes", "Axes missing ylabel", "Add ylabel or verify this is intentional"))
                if len(ax.lines) == 0 and len(ax.collections) == 0 and len(ax.patches) == 0 and len(ax.images) == 0:
                    issues.append(QualityIssue("error", "axes", "Axes is empty (no plotted data)", "Add data to this axes or remove it"))
        return issues
        ...
        for ax in fig.axes:
            for im in ax.images:
                ...
                if cmap_name.lower() in ["jet", "rainbow", "hsv"]:
                    issues.append(QualityIssue("warning", "colors", f"Colormap '{cmap_name}' is not colorblind-safe", "Use 'viridis', 'cividis', or other perceptually uniform colormap"))
            line_colors = []
            ...
            if "#ff0000" in line_colors and "#00ff00" in line_colors:
                issues.append(QualityIssue("warning", "colors", "Red and green lines used together (colorblind-unfriendly)", "Use Okabe-Ito palette or add distinct markers"))
        return issues
        ...
        for ax in fig.axes:
            if ax.axison:
                ...
                if long_labels and len(long_labels) > 3:
                    ...
                    if rotation == 0:
                        issues.append(QualityIssue("info", "labels", f"Long x-tick labels ({len(long_labels)} labels >8 chars) without rotation", "Rotate x-tick labels by 45 degrees"))
        return issues
    ...
    def _check_panel_labels(self, fig):
        ...
        if n_axes > 1:
            ...
            if panel_labels_found == 0 and n_axes > 1:
                issues.append(QualityIssue("info", "panel_labels", f"Multi-panel figure ({n_axes} axes) without panel labels (A, B, C...)", "Add panel labels using add_panel_label()"))
            elif 0 < panel_labels_found < n_axes:
                issues.append(QualityIssue("warning", "panel_labels", f"Inconsistent panel labels: {panel_labels_found}/{n_axes} axes labeled", "Ensure all panels have consistent labels"))
        return issues

    def _check_tight_layout(self, fig):
        issues = []
        if fig._suptitle is not None:
            issues.append(QualityIssue("info", "layout", "Figure has suptitle - verify no overlap with panel titles", "Use fig.tight_layout(rect=[0, 0, 1, 0.95]) to leave room for suptitle"))
        return issues

    @staticmethod
    def report(issues, output_path=None):
        lines = ["=" * 60, "SCIENTIFIC FIGURE QUALITY REPORT", "=" * 60]
        ...
        report = "\n".join(lines)
        if output_path: Path(output_path).write_text(report, encoding="utf-8")
        return report
    ...
    def get_issue_count(issues):
        return {"error": sum(1 for i in issues if i.level == "error"), "warning": sum(1 for i in issues if i.level == "warning"), "info": sum(1 for i in issues if i.level == "info")}
