...
def plot_line(x, y_list, labels=None, xlabel="", ylabel="", title="",
    ...
    if panel_label: add_panel_label(ax, panel_label)
...
def plot_bar(categories, values_list, labels=None, xlabel="", ylabel="", title="",
    ...
    if horizontal:
        ...
        ax.set_ylabel(ylabel)
    if title: ax.set_title(title)
    if log_y and not horizontal: ax.set_yscale("log")
    if ylim: ax.set_ylim(ylim)
    if legend and n_series > 1: ax.legend(frameon=False)
    if grid: ax.grid(True, alpha=0.3, linewidth=0.5, axis="y")
    if panel_label: add_panel_label(ax, panel_label)
    if output_path: save_figure(fig, output_path)
    return fig, ax
...
def plot_boxplot(data_list, labels=None, xlabel="", ylabel="", title="",
    ...
    if title: ax.set_title(title)
    if ylim: ax.set_ylim(ylim)
    if labels: ax.set_xticklabels(labels, rotation=45, ha="right")
    if panel_label: add_panel_label(ax, panel_label)
    if output_path: save_figure(fig, output_path)
    return fig, ax
