...
def main():
    ...
    gen.set_axis_labels(
        ...
        title="Fluorescence decay under UV exposure",
    )
    axes[0].legend(frameon=False, loc="upper right")

    # Panel B: Bar plot - autofluorescence by polymer
    gen.add_bar_plot(
        axes[1], polymers, [fluorescence],
        colors=["#0072B2"],
    )
    ...
    gen.set_axis_labels(
        axes[1],
        xlabel="Polymer type",
        ylabel="Autofluorescence (a.u.)",
        title="Intrinsic autofluorescence by polymer",
    )

    # Panel C: Mechanism diagram - fluorescence sources
    gen.add_mechanism_chain(
        axes[2],
        [
